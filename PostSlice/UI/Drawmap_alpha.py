...
def draw_mask_colormap_alpha(masks, image_shape):  # code trying to use direct-reshape as the method for adding color to the final mask for reducing time
    ...
    color_arr = fast_drawcmap_alpha(mask_labs, mask_inds, rand_color, masks.shape[1])

    color_img = color_arr.reshape((image_shape[0],image_shape[1],4))  # reshape the output image to the correct image shape for the sake of further plt.imshow to show as the color map
...
def fast_drawcmap_alpha(mask_labs_arr, mask_inds_arr, colors, pixel_len):
    '''
    ...
    '''
    color_arr = np.zeros((pixel_len,4), dtype=np.uint8)
