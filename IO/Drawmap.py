...
######-------------------------------- Temporal sealing initial code for drawing masks -------------------------------------

# def draw_mask_colormap(masks, image_shape):
#     '''Drawing segmented masks using randomly selected color
...
#         onemask_list = list(masks[iter_mask].keys())

#         rand_color = get_random_color()  # generate a random color in the form of [R,G,B] for each independent mask

#         for iter_pix in range(len(onemask_list)):
#             pix_ind = onemask_list[iter_pix][1]
#             pix_h = pix_ind // image_shape[1]
#             pix_w = pix_ind - image_shape[1]*pix_h

#             color_img[pix_h,pix_w][:] += rand_color


#         # maskarr = masks[iter_mask].A 
#         # maskmat = maskarr.reshape(image_shape)  # recovering the original iamge shape of the obtained masks
#         # posis = np.where(maskmat>0)

#         # rand_color = get_random_color()  # generate a random color in the form of [R,G,B] for each independent mask
    
#         # for iter_posi in posis:  # coloring all pixels in this mask to the color generated randomly
#         #     # print(iter_posi)
#         #     # print('----------')
#         #     color_img[iter_posi[0],iter_posi[1]][:] = rand_color
