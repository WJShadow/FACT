...
def COM_sp(masks: sp.csr_matrix, imgshape_2d: np.ndarray):
    ...
    '''
    masks = masks.tocsr(copy=True)
    masks.eliminate_zeros()
    num_masks = masks.shape[0]
    COMs = np.zeros((num_masks,2), dtype=np.float32)
    com_paral_calc(
        indptr=masks.indptr,
        posi_ind=masks.indices,
        COMs=COMs,
        col_num=imgshape_2d[1],
        mask_num=num_masks,
    )
...
def com_paral_calc(indptr, posi_ind, COMs, col_num, mask_num):
    ...
    # Parallelize over rows, not pixels.  The former implementation updated
    # shared COM/count slots from a pixel-level ``prange`` and therefore had a
    # data race whenever several pixels belonged to the same mask.  That race
    # changed merge decisions between otherwise identical post-processing
    # runs.  Each iteration below owns one output row, so the result is both
    # parallel and deterministic.
    for i_mask in prange(mask_num):
        start = indptr[i_mask]
        stop = indptr[i_mask + 1]
        count = stop - start
        if count <= 0:
            continue
        sum_h = 0.0
        sum_w = 0.0
        for i_pix in range(start, stop):
            position = posi_ind[i_pix]
            sum_h += position // col_num
            sum_w += position % col_num
        COMs[i_mask, 0] = sum_h / count
        COMs[i_mask, 1] = sum_w / count
