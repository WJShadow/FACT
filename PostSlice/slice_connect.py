...
def slice_connect(infer_3d:np.ndarray, min_area:np.int32, max_area:np.int32, num_workers=0):
    ...
    # ``vstack([])`` below is undefined for an empty time axis.  Treat an
    # empty inference volume as a valid empty result so the Release pipeline's
    # ``empty_safe`` path can preserve its spatial shape and row alignment.
    if infer_3d.shape[0] == 0:
        pixel_count = int(infer_3d.shape[1] * infer_3d.shape[2])
        return (
            sp.csr_matrix((0, pixel_count), dtype=np.int64),
